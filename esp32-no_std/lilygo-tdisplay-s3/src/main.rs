//! LilyGo T-Display S3 — Slint UI demo with dark mode toggle.
//!
...
async fn main(spawner: embassy_executor::Spawner) {
    ...
    // GPIO4: PIN_BAT_VOLT — battery voltage via 11dB ADC, ×2 divider.
    ...
    let mut bat_pin = adc_config.enable_pin(peripherals.GPIO4, Attenuation::_11dB);
    ...
    loop {
        ticker.next().await;
        let reading = adc.read_blocking(&mut bat_pin);
        let battery_mv = if reading > 4000 { -1 } else { (reading as i32 * 6200) / 4095 };
        ui.set_battery_mv(battery_mv);
        info!("bat={}mV (raw={})", battery_mv, reading);
    }
}
